...
const triangleCoords = computed(()=>
{
    ...
    return `${(props.element.x[1]+2)*editorDataStore.zoom}, ${props.element.y[1]*editorDataStore.zoom}, ${(props.element.x[1]-22)*editorDataStore.zoom}, ${(props.element.y[1]+6)*editorDataStore.zoom}, ${(props.element.x[1]-22)*editorDataStore.zoom}, ${(props.element.y[1]-6)*editorDataStore.zoom}`
})
const triangleRotationAngle = computed(()=>
{
    
    let angle = angleBetween2Vectors(bezierPointX.value,bezierPointY.value, props.element.x[1]*editorDataStore.zoom, props.element.y[1]*editorDataStore.zoom);
...
<template>
    ...
    <polygon  :class="{'selected':editorDataStore.selectedElement?.id == props.element.id}" :points="triangleCoords" fill="black" :transform="`rotate(${triangleRotationAngle},${props.element.x[1]*editorDataStore.zoom},${props.element.y[1]*editorDataStore.zoom})`" />
    <circle class="bezier-point" :class="{'active':editorDataStore.selectedElement?.id == props.element.id && editorDataStore.draggingMode == 'edge'}" @click.stop="" @mousedown.stop="startMovingBezierPoint" @mouseup.stop="stopMovingBezierPoint" v-if="editorDataStore.selectedElement?.id == element.id"  :cx="bezierPointX" :cy="bezierPointY" stroke="gray"  r="7"  fill="white" />
</template>
